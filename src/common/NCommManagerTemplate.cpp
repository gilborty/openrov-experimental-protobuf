...
SendResponse NCommManager::SendMessage(ROVMessage& messageOut)
{
    ...
    switch(messageOut.messageType)
    {
        ...
        for message in messages:
            ...
            cog.outl("\tmaster.which_payload = MasterMessage_%s_tag;" % first_letter_to_lower(message))
            cog.outl("\tmaster.payload.%s = *static_cast<%s*>(messageOut.GetData());" % (first_letter_to_lower(message), message))
            cog.outl("\tbreak;")
            cog.outl("}")
        ]]]*/
        //
...
{    
    ...
    switch(static_cast<EMessageType>(decodedMessage.type))
    {
        ...
        for message in messages:
            ...
            cog.outl("\tHandleRecievedMessage(decodedMessage.payload.%s);" % first_letter_to_lower(message))
            cog.outl("\tbreak;")
            cog.outl("}")
        ]]]*/
        //
